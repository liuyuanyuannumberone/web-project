...
QUnit.assert = Assert.prototype = {
	...
	expect: function( asserts ) {
		if ( arguments.length === 1 ) {
			this.测试图片.expected = asserts;
		} else {
			return this.测试图片.expected;
		}
	},
	...
	pushResult: function( resultInfo ) {
		...
		// resultInfo = { result, actual, expected, message, negative }
		var assert = this,
			currentTest = ( assert instanceof Assert && assert.测试图片 ) || QUnit.config.current;
		...
		return assert.测试图片.pushResult( resultInfo );
	},
	...
	"throws": function( block, expected, message ) {
		var actual, expectedType,
			...
			ok = false,
			currentTest = ( this instanceof Assert && this.测试图片 ) || QUnit.config.current;
		...
		if ( actual ) {
			...
			// expected is a regexp
				...
				ok = expected.测试图片( errorString( actual ) );
